...
# --------------------------------------------------------------------------- #
def fetch_all(g):
    """Pull everything and return the data dict (no file I/O). Reused by serve.py."""
...
def write_data_js(data, path=OUT_FILE):
...
def main():
    ...
    print(f"\n✓ Wrote {OUT_FILE} "
          ...
          "\n  Open dashboard.html to view your data, or run `python serve.py` for"
          " an auto-refreshing live view.")
